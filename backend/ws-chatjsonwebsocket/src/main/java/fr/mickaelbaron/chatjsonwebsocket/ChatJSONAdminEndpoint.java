...
/**
 * Gestion de l'authentification et envoie la liste des chatrooms pour admins
 * 
 * @author teulierf
 * @version 3.1.0
 * @see BE-SAVE
...
public class ChatJSONAdminEndpoint {
    ...
    public void onOpen(Session session, @PathParam("username") String userName, @PathParam("password") String password) throws IOException {
		...
		//Renvoie l'admin s'il existe ou NULL sinon, grace au username
        ...
        if (adminExistant != null) {
            ...
            System.out.println("Admin existant:" + adminExistant.getRole() + ", " + adminExistant.getUserId());
        ...
        System.out.println("Nouvel admin valide :" + nouvelUtilisateur.getRole() + ", " + nouvelUtilisateur.getUserId());
        ...
        broadcastListChatroom(infoMessage, session);
    		...
    		//Admin non autorisé
    		System.out.println("Admin non autorisé");
    	       session.close();
    	       return;
    	}
    ...
    //Envoie des notif de messsages non lu
